...
static CAProtocolCtx caProto = {.undefined        = ACTenChannelInputHandler,
...
static void GpioInit() {
    static GPIO_TypeDef* const pinsBlk[] = {
        CTRL_0_GPIO_Port, CTRL_1_GPIO_Port, CTRL_2_GPIO_Port, CTRL_3_GPIO_Port, CTRL_4_GPIO_Port,
...
static void updateBoardStatus() {
    ...
    for (int i = 0; i < AC_TEN_CH_NUM_PORTS; i++) {
        stmGetGpio(heaterPorts[i]) ? bsSetField(AC_TEN_CH_PORT_x_STATUS_Msk(i))
                                   : bsClearField(AC_TEN_CH_PORT_x_STATUS_Msk(i));
    }
